...
class Puzzle:
    ...
    def linearConflictHeuristic(self):
        def manhattanDistance(pos1, pos2):
            return abs(pos1[0] - pos2[0]) + abs(pos1[1] - pos2[1])

        def conflicts(row, goal_row):
            count = 0
            for i in range(self.size - 1):
                for j in range(self.size - 1):
                    if i >= len(row) or j >= len(row or j >= len(goal_row)):
                        continue
                    if row[i] != '' and row[i] == goal_row[j]:
                        for k in range(j + 1, self.size - 1):
                            if (k >= len(row)):
                                continue
                            if row[k] != '' and row[k] == goal_row[i] and manhattanDistance((i, k),
                                                                                            (j, k)) < manhattanDistance(
                                    (i, k), (k, k)):
                                count += 2
            return count
        ...
        heuristic_value = 0
        for i in range(1, self.size - 1):
            row = self.puzzle[i][1:self.size - 1]
            goal_row = self.goal_state[i - 1]
            heuristic_value += conflicts(row, goal_row)
        for j in range(1, self.size - 1):
            col = [self.puzzle[i][j] for i in range(1, self.size - 1)]
            goal_col = [self.goal_state[i - 1][j - 1] for i in range(self.size - 2)]
            heuristic_value += conflicts(col, goal_col)
