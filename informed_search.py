...
class AStarSearchGraph:
    ...
    def searchLinearConf(self):
        heapq.heappush(self.frontier, (
        self.initial_state.path_cost + self.initial_state.linearConflictHeuristic(), self.initial_state))
        generated_nodes = 0
        expanded_nodes = 0
        visited = []
        ...
        while self.frontier:
            ...
            for move in current_state.get_possible_moves():
                new_state = current_state.copy()
                new_state.applyMove(move)
                cost = new_state.path_cost + new_state.linearConflictHeuristic()
...
class IDA:
    ...
    def searchLinearConf(self):
        self.pruned = []
        self.threshold = self.initial_state.path_cost + self.initial_state.linearConflictHeuristic()
        heapq.heappush(self.pruned, (
        self.initial_state.path_cost + self.initial_state.linearConflictHeuristic(), self.initial_state))
        visited = 0
        c = 0
        while True:
            c += 1
            result = self.LinearConf_search_depth(self.initial_state, self.initial_state.path_cost)
            if result:
                print("Goal state reached!")
    ...
    def LinearConf_search_depth(self, current_state, g, visited=0):
        visited += 1
        f = g + current_state.linearConflictHeuristic()
        ...
        for move in current_state.get_possible_moves():
            ...
            child_result = self.LinearConf_search_depth(new_state, g + 1, visited)
